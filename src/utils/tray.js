...
function findSolvableTray(board, memo) {
...
// Replace the given tray slots with new random pieces, keeping the tray solvable.
export function rerollSlots(board, tray, slotIndices) {
  ...
  for (let attempt = 0; attempt < RANDOM_TRAY_ATTEMPTS; attempt += 1) {
    const candidateTray = tray.map((piece, index) =>
      targets.has(index) && piece ? createPieceInstance(randomPieceTemplate()) : piece
    );
  ...
  return nextTray(board);
}

// Build `count` random piece instances to offer as reroll replacement candidates.
export function rerollCandidates(count) {
  return Array.from({ length: count }, () => createPieceInstance(randomPieceTemplate()));
}

// Every piece template as a fresh instance, for the "pick from the deck" reroll level.
export function deckPieceInstances() {
  return PIECES.map(createPieceInstance);
}

// Replace a single random non-empty tray slot with a new piece, keeping the tray solvable.
export function rerollOnePiece(board, tray) {
  ...
  for (let attempt = 0; attempt < RANDOM_TRAY_ATTEMPTS; attempt += 1) {
    const candidateTray = withCandidate(randomPieceTemplate());
    if (canCompleteTray(board, candidateTray.filter(Boolean))) {
      return candidateTray;
    }
  }

  // Exhaustively look for any template that keeps the tray completable.
...
export function nextTray(board) {
  ...
  return findSolvableTray(board, memo).map(createPieceInstance);
}
